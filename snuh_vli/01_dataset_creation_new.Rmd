...
hb_diff = value
hb_diff$value = hb_diff$value.y- hb_diff$value.x
...
```
```{r}
table1(~ sex + age + hosp_mortality + value.x + value.y +fluid_balance + value + leaking_index | q_leaking_index, data=selected_df_mortality)
```
