-- ref.: https://github.com/MIT-LCP/mimic-code/blob/bigquery_load_data/concepts/durations/ventilation_durations.sql
...
select stay_id
  , CASE WHEN DATETIME_DIFF(max(charttime), min(charttime), MINUTE)/60 >0 THEN 1 ELSE 0 END AS mech_vent_true
from vd2
group by stay_id
