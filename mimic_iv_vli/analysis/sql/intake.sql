...
ON
  (icustays.stay_id = metavision_realtime_final.stay_id) AND metavision_realtime_final.chartoffset BETWEEN -6*60 AND 36*60 
LEFT JOIN
 metavision_intake_final
ON
  (icustays.stay_id = metavision_intake_final.stay_id) AND metavision_intake_final.chartoffset BETWEEN -6*60 AND 36*60
WHERE
