...
where amount > 248 and icustay_id is not null
...
-- just because the rate was high enough, does *not* mean the final amount was
where amount > 248 and icustay_id is not null
group by t4.icustay_id, t4.charttime, intime
UNION distinct
select
    icustay_id
  , charttime
...
using (icustay_id)
where icustay_id is not null
group by t5.icustay_id, t5.charttime, intime
...
t6
using (icustay_id)
where intake_first is not null and chartoffset BETWEEN -6*60 AND 36*60
group by icustay_id, chartoffset
order by icustay_id)
select
icustay_id,
sum (intakes) as intakes
from
t7
group by icustay_id
order by icustay_id;