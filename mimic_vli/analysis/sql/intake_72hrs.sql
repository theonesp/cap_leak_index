...
ON
  (icustays.icustay_id = metavision_realtime_final.icustay_id) AND metavision_realtime_final.chartoffset BETWEEN 36*60 AND 84*60 
LEFT JOIN
 metavision_intake_final
ON
  (icustays.icustay_id = metavision_intake_final.icustay_id) AND metavision_intake_final.chartoffset BETWEEN 36*60 AND 84*60
WHERE
...
ON
  (icustays.icustay_id = careveu_realtime_final.icustay_id) AND careveu_realtime_final.chartoffset BETWEEN 36*60 AND 84*60 
LEFT JOIN
 careveu_intake_final
ON
  (icustays.icustay_id = careveu_intake_final.icustay_id) AND careveu_intake_final.chartoffset BETWEEN 36*60 AND 84*60
WHERE
  tev IS NOT NULL 
OR
