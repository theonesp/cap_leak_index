...
print('Number of patients after excluding patients with missing hosp_mortality:')
b<-nrow(selected_df)
a-b

selected_df<-selected_df%>%filter(!is.na(age_fixed))
print('Number of patients after excluding patients with missing age:')
c<-nrow(selected_df)
b-c

selected_df<-selected_df%>%filter(!is.na(gender))
print('Number of patients after excluding patients with missing gender:')
d<-nrow(selected_df)
c-d

selected_df<-selected_df%>%filter(!is.na(final_elixhauser_score))
print('Number of patients after excluding patients with missing final_elixhauser_score:')
e<-nrow(selected_df)
d-e

selected_df<-selected_df%>%filter(!is.na(oasis))
print('Number of patients after excluding patients with missing oasis:')
f<-nrow(selected_df)
e-f

selected_df<-selected_df%>%filter(!is.na(leaking_index))
print('Number of patients after excluding patients with missing leaking_index:')
g<-nrow(selected_df)
f-g

selected_df<-selected_df%>%filter(!is.na(delta_sofa))
print('Number of patients after excluding patients with missing delta_sofa:')
h<-nrow(selected_df)
g-h
...
```{r}
table1(~ age_fixed + hosp_mortality + gender + final_elixhauser_score + oasis  + delta_sofa  | q_leaking_index, data=selected_df)
```
