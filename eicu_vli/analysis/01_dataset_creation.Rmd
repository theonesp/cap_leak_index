...
```
```{r}
hist(selected_df_fluid$fluid_balance_72, breaks = 500, xlim = c(0, 10000))
```
...
```{r}
vars_in_table1<-c('age_fixed', 'gender'  , 'final_charlson_score' , 'apache_iv'  , 'delta_sofa' ,'sofatotal_day1_cat', 'fluid_balance' , 'mean_hct_24_36hrs' , 'first_hct_6hrs', 'mech_vent','hosp_mortality_offset', 'hosp_mortality', 'actualicumortality' , 'unabridgedunitlos' , 'unabridgedhosplos','leaking_index')
...
```{r}
eICUTable1 <- table1(~ age_fixed + hosp_mortality + gender + final_charlson_score + apache_iv  + delta_sofa + fluid_balance_72 + fluid_balance + mean_hct_24_36hrs + first_hct_6hrs | q_leaking_index, data=selected_df_fluid)
eICUTable1
```
