...
# 
# print('patients with positive leak index')
# nrow(vol_leak_index_dataset)
```
...
## We are removing 72hr fluid balance outliers
# outliers <- boxplot(vol_leak_index_dataset$fluid_balance_72, plot=FALSE)$out
# vol_leak_index_dataset$fluid_balance_72[which(vol_leak_index_dataset$fluid_balance_72 %in% outliers)]<-NA
```
...
hist (vol_leak_index_dataset$fluid_balance_72, main = 'fluid')
...
```
```{r}
plot (vol_leak_index_dataset$fluid_balance, vol_leak_index_dataset$fluid_balance_72)
linearMod <- lm(fluid_balance_72 ~ fluid_balance, data=vol_leak_index_dataset)
summary(linearMod)
```
```{r}
