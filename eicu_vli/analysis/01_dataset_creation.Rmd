...
Not addressing outliers anymore

```{r eval=FALSE, include=FALSE}

# We are removing fluid balance outliers
#outliers <- boxplot(vol_leak_index_dataset$fluid_balance, plot=FALSE)$out
# vol_leak_index_dataset$fluid_balance[which(vol_leak_index_dataset$fluid_balance %in% outliers)]<-NA

## We are removing VLI outliers
outliers <- boxplot(vol_leak_index_dataset$leaking_index, plot=FALSE)$out
#vol_leak_index_dataset$leaking_index[which(vol_leak_index_dataset$leaking_index %in% outliers)]<-median(vol_leak_index_dataset$leaking_index,na.rm = T)

#how many patients are we imputing?

extreme_quants<-as.numeric(quantile(vol_leak_index_dataset$leaking_index, c(.05, .95),na.rm = T))

vol_leak_index_dataset%>%
  filter(leaking_index< extreme_quants[1] |leaking_index>extreme_quants[2])%>%
  tally()


vol_leak_index_dataset<-vol_leak_index_dataset%>%
  mutate(leaking_index=
           if_else(leaking_index< extreme_quants[1] | leaking_index>leaking_index[2], median(leaking_index,na.rm = T),leaking_index)
         )

# vol_leak_index_dataset$leaking_index<-Winsorize(vol_leak_index_dataset$leaking_index,na.rm = T)  
# quantile(vol_leak_index_dataset$leaking_index, c(.05, .95),na.rm = T)
```
...
selected_df<-selected_df%>%filter(hosp_mortality_offset<72*60)
...
selected_df<-selected_df%>%
  mutate(
     q_leaking_index=as.factor(as.numeric(cut2(leaking_index, g=8)))
)
```
...
print('cutoff points')
unique(cut2(selected_df$leaking_index, g=8))
```
...
```{r}
table1(~ age_fixed + hosp_mortality + gender + final_charlson_score + apache_iv  + delta_sofa + fluid_balance + mean_hct_24_36hrs + first_hct_6hrs| q_leaking_index, data=selected_df)
```
