...
```{r}
plot (selected_df_fluid$fluid_balance, selected_df_fluid$hosp_mortality, xlim = c(0, 25000))
linearMod <- lm(hosp_mortality ~ fluid_balance, data=selected_df_fluid)
summary(linearMod)
```
# 72hr Fluid Regression Analysis
```{r}
plot (selected_df_fluid$leaking_index, selected_df_fluid$fluid_balance_72, xlab = 'leaking_index', ylab = 'fluid balance at 72hrs', main = 'Leaking Index as a predictor of Fluid Balance at 72 hrs')
linearMod <- lm(fluid_balance_72 ~ leaking_index, data=selected_df_fluid)
summary(linearMod)
```

# 72hr Fluid Regression Based on Quartiles
```{r}
linearMod <- lm(fluid_balance_72 ~ q_leaking_index + age_fixed + gender + fluid_balance + apache_iv + final_charlson_score, data=selected_df_fluid)
summary(linearMod)
```
