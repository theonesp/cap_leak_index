...
ps_model <- glm( highest_q_leaking_index ~ 
                  center_scale(log(apache_iv))
                + age_fixed
                + gender
                + log(final_charlson_score+1)
                + log(totalFluid)
                , family=binomial()
...
weighedtable<- svyCreateTableOne( vars= c("age_fixed", "gender","final_charlson_score","predictedHospitalMortality"), strata = "highest_q_leaking_index", data = weighted_data, test = F)
