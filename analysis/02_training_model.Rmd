...
cap_leak_index_dataset$actualhospitalmortality[cap_leak_index_dataset$actualhospitalmortality=='ALIVE']<-0
cap_leak_index_dataset$actualhospitalmortality[cap_leak_index_dataset$actualhospitalmortality=='EXPIRED']<-1
cap_leak_index_dataset$actualhospitalmortality<-as.factor(cap_leak_index_dataset$actualhospitalmortality)
mod_multiv<-glm(actualhospitalmortality  ~ 
                 q_leaking_index
                +apachescore
                +final_charlson_score
                 ,family='binomial'
                 ,data = cap_leak_index_dataset
                )
mod_multiv
...
auc(cap_leak_index_dataset$actualhospitalmortality, cap_leak_index_dataset$apachescore)
Predicted = predict (mod_multiv, cap_leak_index_dataset)
auc(cap_leak_index_dataset$actualhospitalmortality, Predicted)
```
...
final_predictions['pred']<-ifelse(final_predictions$ALIVE > .9 # we have set the threshold in .5 this can be optimized until best performance is achieved
                                  , 'ALIVE','EXPIRED'
)
...
evaluation_insights$Insights <- factor(evaluation_insights$Insights, levels = unique(evaluation_insights$Insights)[order(evaluation_insights$Insights, decreasing = T)])
...
p <- plot_ly(
  ...
  textposition='auto',
  type = "bar") %>%
  layout( title = paste(best_performing_model_name,"Model Insights"))
...
# Odds ratio (work in progress)

Valid for gam logreg and glm.

```{r}
oddsratio::or_gam(data=train,model= best_performing_model)
