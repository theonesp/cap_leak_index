...
selected_model<-gbmFit
selected_model_name<-deparse(substitute(gbmFit)) # extracts name as string from model
...
final_predictions['pred']<-ifelse(final_predictions$ALIVE > .4 # we have set the threshold in .5 this can be optimized until best performance is achieved
                                  , 'ALIVE','EXPIRED'
)
