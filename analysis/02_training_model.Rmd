...
# Selecting variables
 ...
 - We are creating the variable highest_q_leaking_index.

```{r}
selected_df <- vol_leak_index_dataset%>%mutate(
  highest_q_leaking_index=if_else(
    q_leaking_index %in% c('4'),1,0
  ))%>%dplyr::select(
    patientunitstayid
    ,hosp_mortality
    ,age_fixed
...
selected_df$hosp_mort<-as.factor(selected_df$hosp_mort)
```
...
```{r}
print('Number of patients in our initial dataset:')
nrow(selected_df)
...
```{r}
table1(~ age_fixed + hosp_mort + gender + final_charlson_score + apache_iv  + delta_sofa  | highest_q_leaking_index, data=selected_df)
```
