...
# Gathers the patients with sepsis and relevant demographics
```{r}
...
# Extracts fluid data
```{r}
fluidIntake <- run_query(getSQL("sql/eicu_intakeV2.sql" ))
names(fluidIntake)[names(fluidIntake) == "f0_"] <- "total_intake"
...
Final_dataset<-Final_dataset%>%
  mutate(
    ...
    leakingn_index=((mean_hct_24_36hrs-first_hct_6hrs) / total_intake)
...
Final_dataset<-Final_dataset%>%
  filter(!is.infinite(leakingn_index))
```
