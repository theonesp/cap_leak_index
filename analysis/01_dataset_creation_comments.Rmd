...
```{r}
# SOFA variables
sofa_cv_open <- run_query(getSQL("sql/sofa/sofa_cv_open.sql"))
sofa_renal_open <- run_query(getSQL("sql/sofa/sofa_renal_open.sql"))
sofa_respi_open <- run_query(getSQL("sql/sofa/sofa_respi_open.sql"))
sofa_3others_open <- run_query(getSQL("sql/sofa/sofa_3others_open.sql"))
```
...
Final_dataset<-Reduce(function(...) merge(..., all.x=TRUE), list(selected_cohort, apache_related,hct,AKIlist_final,charlson_score
))
# sofa_eicu
sofa_eicu<-sqldf('
SELECT sofa_cv_open.patientunitstayid,
...
INNER JOIN sofa_respi_open  ON sofa_cv_open.patientunitstayid = sofa_respi_open.patientunitstayid
INNER JOIN sofa_renal_open  ON sofa_cv_open.patientunitstayid = sofa_renal_open.patientunitstayid
INNER JOIN sofa_3others_open  ON sofa_cv_open.patientunitstayid = sofa_3others_open.patientunitstayid
GROUP BY sofa_cv_open.patientunitstayid
ORDER BY sofa_cv_open.patientunitstayid
')
