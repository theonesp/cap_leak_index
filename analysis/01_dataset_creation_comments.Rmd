...
```
``` {r}
all_reliable <- run_query (getSQL("sql/sofa/reliable_select.sql"))
```
```{r}
...
infectAndOrgan = run_query(getSQL('sql/Alistair_sepsis.sql'))
patient_inexcluded_icd9 <- merge (infectAndOrgan, patient_inexcluded_icd9, all = TRUE)
```
...
Final_dataset<-Final_dataset%>%
  mutate(
    ...
    ,delta_sofa=sofatotal_day1-sofatotal_day4
...
listVarstab1<-c("patientunitstayid", "age_fixed", "gender", "weight", "height", "BMI", "BMI_group", "unitType", "body_surface_area", "apachescore", "unabridgedunitlos", "unabridgedhosplos", "unabridgedactualventdays", "first_hct_6hrs", "mean_hct_24_36hrs", "AKIstage", "delta_sofa", "delta_sofa_bin", "actualhospitalmortality", "HospitalLOS")

catVars<-c("gender", "actualicumortality", "actualhospitalmortality", "delta_sofa_bin")

mortality <- CreateTableOne(vars = listVarstab1, data = Final_dataset, factorVars = catVars,testNormal=oneway.test, strata="Q_leakingn_index")
write.csv(as.data.frame(print(mortality)), "mortality.csv", col.names=TRUE, row.names=TRUE)
```
