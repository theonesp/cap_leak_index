...
## Apache Median imputation

```{r}
selected_df<-selected_df%>%
  mutate(
    apache_iv=impute.median(apache_iv)
  )  
```
...
selected_df<-selected_df%>%
  mutate(
    hosp_mortality=if_else(is.na(hosp_mortality),0,as.double( hosp_mortality))
  )  
```
...
selected_df<-selected_df%>%mutate(
  highest_q_leaking_index=if_else(q_leaking_index %in% c('4','3'),1,0 )
)
...
*Stratified by highest_q_leaking_index*

```{r}
table1(~ age_fixed + hosp_mortality + gender + final_charlson_score + apache_iv  + delta_sofa  | highest_q_leaking_index, data=selected_df)
```
