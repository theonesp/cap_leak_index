...
```{r}
# 100/3 is for a unit conversion
vol_leak_index_dataset<-vol_leak_index_dataset%>%
  mutate(
    leaking_index=((mean_hct_24_36hrs - first_hct_6hrs) * (100/3) / totalFluid) * body_surface_area * 1561
  )  
```
...
selected_df_fluid <- vol_leak_index_dataset%>%dplyr::select(
    ...
    ,gender
    ,charlson_score
    # the data availability for apache score and sofa is too low
    # ,apachescore
    # ,delta_sofa
...
```{r}
table1(~ gender + age_fixed + hosp_mortality + totalFluid + mean_hct_24_36hrs + first_hct_6hrs + charlson_score| q_leaking_index, data=selected_df_mortality)
```
## Summary of dataset
```{r}
