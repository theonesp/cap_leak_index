...
project_id <- "amsterdam-translation" #hst-953-2018
...
# Joining the Amsterdam UMC DB and Combining the datasets

```{r}
amsterdam = run_query(getSQL("sql/amsterdam_sql/combined.sql" ))
...
amsterdam$hosp_mortality = amsterdam$actualhospitalmortality
amsterdam$q_leaking_index = ntile(amsterdam$leaking_index,4)
amsterdam$highest_q_leaking_index = ntile(amsterdam$leaking_index,2)
...
selected_df$age_fixed = vapply(selected_df$age_fixed,age_grouper,character(1)) 

amsterdam = amsterdam[names(selected_df)]
selected_df['db_source'] = 'eicu'
amsterdam['db_source'] = 'amsterdam'
combined = rbind(selected_df,amsterdam)
```
...
```{r}
table1(~ factor(hosp_mortality) + age_fixed + factor(hosp_mortality) + gender + final_charlson_score + apachescore  + delta_sofa + totalFluid | highest_q_leaking_index, data=combined)
```
...
selected_df = combined # for compatibility with code below
```
