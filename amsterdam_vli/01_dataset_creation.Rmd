...
vol_leak_index_dataset$hosp_mortality = vol_leak_index_dataset$actualhospitalmortality
...
### Addressing VLI and Fluid outliers (not anymore)
...
table1(~ gender + age_fixed + hosp_mortality + totalFluid + totalFluid_72 + mean_hct_24_36hrs + first_hct_6hrs + charlson_score| q_leaking_index, data=selected_df)
```
...
linearMod <- lm(totalFluid_72 ~ q_leaking_index, data=selected_df)
summary(linearMod)
```
