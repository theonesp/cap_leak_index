...
# Model performance
data.frame(
  RMSE = RMSE(predictions, selected_df$totalFluid_72),
  R2 = R2(predictions, selected_df$totalFluid_72)
)

ggplot(selected_df, aes(leaking_index, totalFluid_72_bin) ) +
  stat_smooth(method = gam, formula = y ~ s(x)) +
