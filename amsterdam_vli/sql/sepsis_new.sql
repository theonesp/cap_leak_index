WITH diagnosis_groups AS (
    ...
    FROM `physionet-data.amsterdamdb.procedureorderitems`
    WHERE 
        itemid IN (
...
FROM `physionet-data.amsterdamdb.admissions` adm
LEFT JOIN diagnoses on adm.admissionid = diagnoses.admissionid
LEFT JOIN diagnosis_subgroups on adm.admissionid = diagnosis_subgroups.admissionid
LEFT JOIN diagnosis_groups on adm.admissionid = diagnosis_groups.admissionid
LEFT JOIN sepsis on adm.admissionid = sepsis.admissionid
LEFT JOIN sepsis_antibiotics on adm.admissionid = sepsis_antibiotics.admissionid
LEFT JOIN other_antibiotics on adm.admissionid = other_antibiotics.admissionid
LEFT JOIN cultures on adm.admissionid = cultures.admissionid
...
WHERE
((surgical = 1 AND (lower(diagnosis) like '%sepsis%' or lower(diagnosis) like '%sepsis%' or 
lower(diagnosis) like '%pneumoni%' or lower(diagnosis) like '%gi perforation%' or
lower(diagnosis) like '%perforation/rupture%' or lower(diagnosis) like '%infection%' or
lower(diagnosis) like '%abscess%' or lower(diagnosis) like '%gi vascular ischemia%' or
lower(diagnosis) like '%diverticular%' or lower(diagnosis) like '%appendectomy%' or
lower(diagnosis) like '%peritonitis%'))
OR
(surgical = 0 AND (lower(diagnosis) like '%sepsis%' or lower(diagnosis) like '%septic%' or 
lower(diagnosis) like '%infect%' or lower(diagnosis) like '%pneumoni%' or
lower(diagnosis) like '%cholangitis%' or lower(diagnosis) like '%pancr%' or
lower(diagnosis) like '%endocarditis%' or lower(diagnosis) like '%meningitis%' or
lower(diagnosis) like '%gi perforation%' or lower(diagnosis) like '%abces%' or
lower(diagnosis) like '%abscess%' or lower(diagnosis) like '%darm ischaemie%' or
lower(diagnosis) like '%gi vascular%' or lower(diagnosis) like '%fasciitis%' or
lower(diagnosis) like '%inflammatory%' or lower(diagnosis) like '%peritonitis%'))
OR
sepsis_at_admission = 1
OR
