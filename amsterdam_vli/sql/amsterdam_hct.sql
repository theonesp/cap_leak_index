...
WITH adm as (
    ...
    measuredat/86400000 as measuredat 
    FROM `physionet-data.amsterdamdb.numericitems`
    WHERE item IN ('Hematocriet','A_Hematocriet(%)','RA_Hematocriet(%)','MCA_Hematocriet(%)','Ht(v.Bgs) (bloed)','Ht (bloed)') AND measuredat BETWEEN 64800000 AND 129600000
),  second_hct2 as (
    SELECT
    admissionid,
    avg(value) as value
   ...
   from `physionet-data.amsterdamdb.admissions`
    ...
    FROM  (
        ...
        AND stop BETWEEN -0.25 AND 1.5 --first 24 hours
--        WHERE ordercategory = "Infuus - Crystalloid"
    ...
    LEFT JOIN (
        ...
        AND measuredat BETWEEN -0.25 AND 1.5 --first 24 hours
        GROUP BY d.admissionid, d.days
    ) fluidout_daily
    ...
    ON d.admissionid = n.admissionid
    AND measuredat/86400000 BETWEEN -0.25 AND 1.5 --first 24 hours
